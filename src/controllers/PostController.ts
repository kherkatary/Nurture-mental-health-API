...
const allPost = async (_req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    else  {  allPost = await postModel.find().sort({_id:-1});}
...
const newPost = async (_req: express.Request, res: express.Response) => {
...
const getPost = async (_req: express.Request, res: express.Response) => {
  const { pid } = _req.body;
...
const deletePost=async(_req: express.Request, res: express.Response)=>{
    const {pid}= _req.body;
    if(!pid)return res.status(400).send({message:"PID required"});
    try{
